...
// नई प्रोफाइल्स फेच करें (पिछले 30 दिनों में जोड़ी गई)
$thirtyDaysAgo = date('Y-m-d H:i:s', strtotime('-30 days'));

// टोटल नई प्रोफाइल्स काउंट करें
$totalProfiles = $db->selectOne("
    SELECT COUNT(*) as count 
    FROM listings 
    WHERE is_active = 1 AND created_at >= :thirty_days_ago
", ['thirty_days_ago' => $thirtyDaysAgo]);

$totalPages = ceil($totalProfiles['count'] / $perPage);

// नई प्रोफाइल्स फेच करें
$newProfiles = $db->select("
    ...
    WHERE l.is_active = 1 AND l.created_at >= :thirty_days_ago
    ORDER BY l.created_at DESC 
    LIMIT $perPage OFFSET $offset
", ['thirty_days_ago' => $thirtyDaysAgo]);
?>

<section class="new-profiles py-4">
    <div class="container">
        <h1 class="section-title mb-4">New Profiles</h1>
        <p class="text-muted mb-4">Profiles added in the last 30 days</p>
        ...
        <div class="row">
            ...
            <?php else: ?>
                <div class="col-12">
                    <div class="alert alert-info">No new profiles found in the last 30 days.</div>
                </div>
            <?php endif; ?>
        </div>
